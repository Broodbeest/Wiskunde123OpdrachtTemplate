<title>Leerjaar 2 | Procenten</title>

<div class="container vaardigheden">

<!--het hoofdblok-->
<div class="bodytekst">
	
<nav>
<ul class="doelen">
	<li>
		<a href="#open_doelen">Doelen</a>
		<div id="open_doelen" class="modalDialog">
			<div>
				<a href="#close" title="Close" class="close">X</a>
				<h3>	Doelen</h3>
				<ol>
					<li>Leerlingen leren de samenhang tussen breuken, verhoudingen en procenten.</li>
					<li>Leerlingen leren breuken en verhoudingen als percentages uit te drukken.</li>
					<li>Leerlingen leren bij deel en geheel de aantallen te berekenen.</li>
					<li>Leerlingen leren bij gegeven percentages de aantallen te berkenen.</li>
				</ol>
			</div>
		</div>
	</li>
	<li>
		<a href="#open_voorkennis">Voorkennis</a>
		<div id="open_voorkennis" class="modalDialog">
			<div>
				<a href="#close" title="Close" class="close">X</a>
				<h3>Voorkennis</h3>
				<ol>
					<li>verhoudingstabellen en breuken;</li>
					<li>decimale getallen en afronden;</li>
					<li>rekenen met rekenmachine.</li>
				</ol>
			</div>
		</div>
	</li>
	<li>
		<a href="#open_werkwijze">Werkwijze</a>
		<div id="open_werkwijze" class="modalDialog">
			<div>
				<a href="#close" title="Close" class="close">X</a>
				<h3>Werkwijze</h3>
				<p>
					Zorg dat je een rekenmachine bij de hand hebt. Gebruik geen telefoon, maar een echte rekenmachine.<br>
				Maak de opdrachten uit de taak in je schrift. Schrijf sommen steeds eerst over voordat je ze stap voor stap uitrekent. Kijk je eigen werk kritisch na. Tip: kijk elkaars werk na en geef elkaar feedback over het gemaakte werk.
				</p>
			</div>
			</div>
	</li>
	<li>
		<a href="#open_benodigdheden">Benodigdheden</a>
		<div id="open_benodigdheden" class="modalDialog">
			<div>
				<a href="#close" title="Close" class="close">X</a>
				<h3>Benodigdheden</h3>
				<p>
					In deze taak heb je nodig:
					<ol>
						<li>Schrift, pen, potlood en gum</li>
						<li>Rekenmachine</li>
					</ol>
				</p>
			</div>
		</div>
	</li>	
</ul>
</nav>

<!-- hoofdtekst -->
<h1>Procenten</h1>
	
<!--	
	<h3>Inhoud</h3>
		<p>
		Procenten:
		<ol>
			<li>
				wat zijn percentages;
			</li>
			<li>
				van procent naar decimaal getal;
			</li>
			<li>
				deel : geheel  x 100 %, hoeveel % is 4 van 10?
			</li>
			<li>
				23,5% van 120 bereken. met factor 0,235 x 120
			</li>
			
		</ol>
	</p>
	
		<p>Doe het via de 1!</p>
		<ol>
			<li>
			Verhoudingstabellen herhalen
			</li>
			<li>
			Zoveel <b>per</b> zoveel
			</li>
			<li>
			Zoveel per honderd (is dus procenten)
			</li>
			<li>
			Zoveel procent van... (gebruik de 1!!!)
			</li>
			<li>
			Vergrotingsfactor (de 1 overslaan)
			</li>
			<li>
			Toepassen
			</li>
		</ol>
			</p>
			
		
		-->
		
<!-- theorie inleiding -->
<div class="tekstblok theorie vaardigheden">
	<h3>Inleiding</h3>
		<img src="../afbeeldingen/vaar_procenten/negatieve_korting.gif" alt="negatieve_korting" width="200" class="rightfloat" />
	<p>
		Procent is een heel veel voorkomend woord. Het komt van het Latijnse woord <em>procentum</em> dat per honderd betekent.<br>
		Vooral liefhebbers van de uitverkoop moeten vaak aan de slag met procenten. Dat korting niet altijd even positief uitvalt, blijkt uit de afbeelding hiernaast.
	</p>
	<p>
		In deze taak ga je leren rekenen met procenten. Je hebt je kennis over verhoudingen, decimale getallen en verhoudingstabellen nodig.
		</p>
</div>

<!-- opdracht 1 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 1 Opfrissen</h3>
	<p>
		Neem over, vul in en reken uit, rond af op twee decimalen de komma. <br>
		Maak gebruik van je rekenmachine.
	</p>
	<ol class="breuken">
		</li>
			<li>
			`5/20 = 1/... = ...`
		</li>
		<li>
			`.../18 = 4/9 approx ...`
		</li>
		<li>
			`21/33 = (...)/11 approx ...`
		</li>
	</ol>
	
		<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol class="breuken">
				</li>
					<li>
					`5/20 = 1/4 = 0,25`
				</li>
				<li>
					`8/18 = 4/9 approx 0,44`
				</li>
				<li>
					`21/33 = 7/11 approx 0,64`
				</li>
			</ol>
		</div>
	</div>
	
</div>

<!-- theorie A -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie A de verhoudingstabel</h3>
	<aside class="tekstballon_theorie" style="width:180px !important; ">
	<table class="verhouding">
		<thead>
			<tr>
				<th></th>
				<th colspan="2">&divide; 5<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>

				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr class="borderbottom">
				<td colspan="2">15</td>
				<td colspan="2">3</td>
			</tr>
			<tr>
				<td colspan="2">35</td>
				<td colspan="2">7</td>
			</tr>		
		</tbody>
		<tfoot>
			<tr>
				<th></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>&divide; 5</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
	</aside>
		<p>
		In een verhoudingstabel mag je het onderste en het bovenste deel met hetzelfde getal vermenigvuldigen of door hetzelfde getal delen. Zo blijft verhouding gelijk.
	</p>
	<p>
		De verhouding `15 : 35` is gelijk aan `3 : 7`. <br>
		Lees "3 staat tot 7". Net als bij breuken vereenvoudig je een verhouding zo ver mogelijk.
	</p>
	<p>
		Zie taak <a href="http://wiskunde.vathorstcollege.nl/index2.php?p=vaar_verhoudingen">Verhoudingen</a> voor meer voorbeelden en uitleg.
	</p>
</div>

<!-- opdracht 2 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 2 Verhoudingstabel</h3>
	<p>
		Per 29 koffiekannen heb je 145 schepjes koffie nodig. Bereken hoeveel schepjes koffie je voor 15 kannen zou gebruiken.
	</p>
	<ol>
		<li>
			Neem de verhoudingstabel over.
			<table class="verhouding" style="background-color:white">
			<thead>
				<tr>
					<th colspan="2"></th>
					<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
					<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
					<th></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td><em>schepjes koffie</em></td>
					<td colspan="2">145</td>
					<td colspan="2"></td>
					<td colspan="2">?</td>
				</tr>	
				<tr class="borderbottom">
					<td><em>aantal kannen</em></td>
					<td colspan="2">29</td>
					<td colspan="2">1</td>
					<td colspan="2">15</td>	
				</tr>			
			</tbody>
			<tfoot>
				<tr>
					<th colspan="2"></th>
					<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
					<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× ...</th>
					<th></th>
				</tr>
			</tfoot>	
		</table>
		</li>
		<li>
			Bedenk hoe je van 29 naar 1 komt en van 1 naar 15. Vul de juiste getallen onder en boven in op de stippellijntes bij de pijlen
		</li>
		<li>
		Bereken met behulp van de verhoudingstabel hoeveel schepjes je nodig hebt voor 15 kannen koffie.
		</li>
	</ol>
	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol start="2">
				<li>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ 29<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th colspan="2">× 15<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>schepjes koffie</em></td>
								<td colspan="2">145</td>
								<td colspan="2">...</td>
								<td colspan="2">75</td>
							</tr>	
							<tr class="borderbottom">
								<td><em>aantal kannen</em></td>
								<td colspan="2">29</td>
								<td colspan="2">1</td>
								<td colspan="2">15</td>	
							</tr>
									
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 29</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 15</th>
								<th></th>
							</tr>
						</tfoot>	
					</table>
					</li>
					<li>
						Volgens deze verhouding heb je voor 15 kannen koffie 75 scheppen koffie nodig.
					</li>
				</ol>
		</div>
	</div>
</div>

<!-- theorie B -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie B Hou van de `1`</h3>
		<aside class="tekstballon_theorie" style="width: 250px !important" >
			<table class="verhouding">
		<thead>
			<tr>
				<th></th>
				<th colspan="2">÷ 17<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× 382<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td colspan="2">8</td>
				<td colspan="2"></td>
				<td colspan="2">?</td>
			</tr>	
			<tr class="borderbottom">

				<td colspan="2">17</td>
				<td colspan="2">1</td>
				<td colspan="2">382</td>	
			</tr>
					
		</tbody>
		<tfoot>
			<tr>
				<th></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 17</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 382</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
	</aside>
		<p>
		In een verhoudingstabel reken je via de `1`.
		</p>
		<p>
			Bepaal onder de route, zodat je boven het antwoord kunt berekenen. Het vakje midden boven laat je leeg.
		</p>
		<p>
			Het antwoord is `8 &divide; 17 &times; 382 approx 180`.
		</p>
</div>

<!-- opdracht 3 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 3 Allerlei verhoudingstabellen</h3>
	<p>
		Reken in de verhoudingstabellen steeds via de `1`. Rond je antwoorden af zoals hoort bij de situatie.
	</p>
	<ol>
		<li>
			<img src="../afbeeldingen/vaar_procenten/love1.png" alt="love1" width="50" class="rightfloat">
			Als je per 26 brieven 174 euro aan postzegels nodig hebt, hoeveel euro aan postzegels heb je dan voor 18 brieven nodig? 
			<table class="verhouding" style="background-color:white">
				<thead>
					<tr>
						<th colspan="2"></th>
						<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
						<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td><em>bedrag in €</em></td>
						<td colspan="2">...</td>
						<td colspan="2"></td>
						<td colspan="2">?</td>
					</tr>	
					<tr class="borderbottom">
						<td><em>aantal brieven</em></td>
						<td colspan="2">26</td>
						<td colspan="2">1</td>
						<td colspan="2">18</td>	
					</tr>
				</tbody>
				<tfoot>
					<tr>
						<th colspan="2"></th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× ...</th>
						<th></th>
					</tr>
				</tfoot>	
			</table>
		</li>
		<li>
			<img src="../afbeeldingen/vaar_procenten/love1.png" alt="love1" width="50" class="rightfloat">
			In 9 huishouden staan 5 fietsen voor de deur. Hoeveel fietsen heb je in totaal bij 72 huishoudens?
		<table class="verhouding" style="background-color:white">
			<thead>
				<tr>
					<th colspan="2"></th>
					<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
					<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
					<th></th>
				</tr>
				</thead>
				<tbody>
					<tr>
						<td><em>fietsen</em></td>
						<td colspan="2">...</td>
						<td colspan="2"></td>
						<td colspan="2">?</td>
					</tr>	
					<tr class="borderbottom">
						<td><em>huishoudens</em></td>
						<td colspan="2">...</td>
						<td colspan="2">1</td>
						<td colspan="2">...</td>	
					</tr>
				</tbody>
				<tfoot>
					<tr>
						<th colspan="2"></th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× ...</th>
						<th></th>
					</tr>
				</tfoot>	
			</table>
		</li>
		<li>
			<img src="../afbeeldingen/vaar_procenten/love1.png" alt="love1" width="50" class="rightfloat">
		Iedere 2 minuten komen er 6 auto's langs. Hoeveel minuten gaan voorbij als er 54 auto's zijn langsgekomen?
			<table class="verhouding" style="background-color:white">
				<thead>
					<tr>
						<th colspan="2"></th>
						<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
						<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td><em>...</em></td>
						<td colspan="2">...</td>
						<td colspan="2"></td>
						<td colspan="2">?</td>
					</tr>	
					<tr class="borderbottom">
						<td><em>...</em></td>
						<td colspan="2">...</td>
						<td colspan="2">1</td>
						<td colspan="2">...</td>	
					</tr>					
				</tbody>
				<tfoot>
					<tr>
						<th colspan="2"></th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× ...</th>
						<th></th>
					</tr>
				</tfoot>	
			</table>
		</li>
	</ol>
	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol>
					<li>
						<table class="verhouding" style="background-color:white">
							<thead>
								<tr>
									<th colspan="2"></th>
									<th colspan="2">÷ 26<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th colspan="2">× 18<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th></th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td><em>bedrag in €</em></td>
									<td colspan="2">174</td>
									<td colspan="2"></td>
									<td colspan="2">120,46</td>
								</tr>	
								<tr class="borderbottom">
									<td><em>aantal brieven</em></td>
									<td colspan="2">26</td>
									<td colspan="2">1</td>
									<td colspan="2">18</td>	
								</tr>
							</tbody>
							<tfoot>
								<tr>
									<th colspan="2"></th>
									<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 26</th>
									<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 18</th>
									<th></th>
								</tr>
							</tfoot>	
						</table>
						Voor 18 brieven heb je €120,46 nodig.
					</li>
					<li>
						<table class="verhouding" style="background-color:white">
							<thead>
								<tr>
									<th colspan="2"></th>
									<th colspan="2">÷ 9<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th colspan="2">× 72<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th></th>
								</tr>
								</thead>
								<tbody>
									<tr>
										<td><em>fietsen</em></td>
										<td colspan="2">5</td>
										<td colspan="2"></td>
										<td colspan="2">40</td>
									</tr>	
									<tr class="borderbottom">
										<td><em>huishoudens</em></td>
										<td colspan="2">9</td>
										<td colspan="2">1</td>
										<td colspan="2">72</td>	
									</tr>
								</tbody>
								<tfoot>
									<tr>
										<th colspan="2"></th>
										<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 9</th>
										<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 72</th>
										<th></th>
									</tr>
								</tfoot>	
							</table>
							Bij 72 huishouden staan dan 40 fietsen voor de deur.
						</li>
					<li>
						<table class="verhouding" style="background-color:white">
							<thead>
								<tr>
									<th colspan="2"></th>
									<th colspan="2">÷ 2<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th colspan="2">× 54<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th></th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td><em>auto's</em></td>
									<td colspan="2">6</td>
									<td colspan="2"></td>
									<td colspan="2">162</td>
								</tr>	
								<tr class="borderbottom">
									<td><em>minuten</em></td>
									<td colspan="2">2</td>
									<td colspan="2">1</td>
									<td colspan="2">54</td>	
								</tr>					
							</tbody>
							<tfoot>
								<tr>
									<th colspan="2"></th>
									<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 2</th>
									<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 54</th>
									<th></th>
								</tr>
							</tfoot>	
						</table>
						Er gaan 162 minuten voorbij.
					</li>
				</ol>
		</div>
	</div>
</div>

<!-- opdracht 4 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 4 Koken</h3>
	<p>
		Jip gaat voor 7 personen pasta koken. Ze gebruikt hiervoor het recept hieronder.
	</p>
	<div class="tekstblok theorie" style="background-color: orange; " > 
	<h2>
     Pasta met gerookte zalm en spinazie
   </h2>
   <p>
	   Ingrediënten voor 4 personen:
	   <img src="../afbeeldingen/vaar_procenten/pasta.jpg" alt="pasta" width="230" class="rightfloat" />
	   <ul>
	    <li>
	      1/2 (500g) pak tagliatelle pasta
	     </li>
	     <li>
	     	50 gr boter
	     </li>
	     <li>
	     	250 ml melk
	    </li>
	    <li>
	      1 eetl witte bloem
	    </li>
	    <li>
	      80 gr versgeraspte Parmezaanse kaas
	     </li>
	     <li>
			 	225g gerookte zalm, gehakt
	    </li>
	    <li>
	      30g verse spinazie, gehakt
	    </li>
	    <li>
	      2 eetl kappertjes
	    </li>
	    <li>
	     	4 eetl zongedroogde tomaten, gehakt
	     </li>
	      <li>
	      	50g gehakte verse oregano
	      </li>
		  </ul>
		  Bron: <a href="http://allrecipes.nl/recept/828/pasta-met-gerookte-zalm-en-spinazie.aspx?o_is=Hub_TopRecipe_2">allrecipes.nl</a>.
   </p>
   </div>
   	
		<p>
			Bereken hoeveel ze van elk ingrediënt nodig heeft. Maak zo nodig gebruik van verhoudingstabellen en schrijf je berekeningen netjes op.
		</p>
		
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<p>
				Voor het afronden heb ik gedaan alsof ik echt zou koken. 
			</p>
			<ul>
		    <li>
		      `500 : 4 &times; 7 = 875`, dus bijna twee pakken tagliatelle pasta
		     </li>
		     <li>
		     	`50 : 4 &times; 7 = 87,5` , dus bijna een ons boter
		     </li>
		     <li>
		     	`250 : 4 &times; 7 = 437,50` , dus bijna een halve liter melk
		    </li>
		    <li>
		      `1 : 4 &times; 7 = 1,75` , dus bijna 2 eetlepels witte bloem
		    </li>
		    <li>
		      `80 : 4 &times; 7 = 140`, dus bijna 150 gram versgeraspte Parmezaanse kaas
		     </li>
		     <li>
				 	`225 : 4 &times; 7 = 393,75` , dus ongeveer 400 gram gerookte zalm, gehakt
		    </li>
		    <li>
		    	`30 : 4 &times; 7 = 52,50` , dus ruim een half ons verse spinazie, gehakt
		    </li>
		    <li>
		    	`2 : 4 &times; 7 = 3,5` , dus bijna 4 eetlepels kappertjes
		    </li>
		    <li>
		     	`4 : 4 &times; 7 = 7` eetlepels zongedroogde tomaten
		     </li>
		     <li>
		     	`50 : 4 &times; 7 = 87,50` , dus bijna een ons gehakte verse oregano
		     </li>
			 </ul>
		</div>
	</div>	
</div>

<!-- opdracht 5 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 5 Per zoveel</h3>
	<p>
		Beantwoord de vragen met behulp van een berekening.
	</p>
	<ol>
		<li>
			Een energiedrankje bevat ongeveer 45 kcal per 100 ml.<br>
			In een blikje zit 250 ml.<br>
			Hoeveel kcal krijg je naar binnen wanneer je één blikje energiedracht opdrinkt? Rond af op hele kilocalorieën.
		</li>
		<li>
			Voor 134 geiten is een veld beschikbaar van 222 m<sup>2</sup>.<br>
			Hoeveel ruimte is er per geit? Rond je antwoord af op één decimaal.
		</li>
		<li>
			De minimumoppervlaktes voor de beschikbare binnenruimte voor geiten is 1,5 m<sup>2</sup> per geit.<br>
			Een binnenruimte is 420 m<sup>2</sup>.<br>
			Voor hoeveel geiten is deze binnenruimte geschikt?
		</li>
	</ol>
	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol>
				<li>
					`45 : 100 &times; 250 = 113` kcal 
				</li>
				<li>
					`222 : 134 approx 1,7` m<sup>2</sup>
				</li>
				<li>
					`420 : 1,5 = 280`<br>
					Een binnenruimte van 420 m<sup>2</sup> zou, volgens deze normen, geschikt zijn voor 280 geiten.
				</li>
			</ol>	
		</div>
	</div>	
</div>
	
<!-- opdracht 6 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 6 Overal procenten</h3>
	<p>
		Overal kom je procenten tegen: in winkels, bij de bank, in kranten, et cetera.<br>
		Schrijf op wat bedoeld wordt met:
	</p>
	<img src="../afbeeldingen/vaar_procenten/100%25.jpeg" alt="100%25" width="120" class="rightfloat" />
	<ol>
		<li>
			Deze stof is 100% wol.
		</li>
		<li>
			We hebben 50% kans om te winnen.
		</li>
		<li>
			Dat is 99% zeker.
		</li>
		<li>
			Ik krijg 3% spaarrente.
		</li>
	</ol>
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">

			<ol>
					<li>
						Dat de stof helemaal van wol gemaakt is.
					</li>
					<li>
						Dat de kans om te winnen even groot is als de kans om te verliezen.
					</li>
					<li>
						Dat iets zo goed als zeker is.
					</li>
					<li>
						Dat elke 100 euro een rente van 3 euro oplevert.
					</li>
				</ol>
			
				<p>
					Jouw antwoorden of de verwoording ervan kunnen afwijken van bovenstaande antwoorden.
				</p>
		</div>
	</div>
</div>

<!-- opdracht 7 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 7 Vergelijken</h3>
	<p>
		Voor Edukans wordt geld ingezameld. Jan heeft 87 euro en geeft 5 euro aan dit goede doel. Maja heeft 114 euro en geeft 6 euro.<br>
		Wie vind jij de gulste gever? Licht je antwoord toe.
	</p>

	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<p class="breuken">
				Vergelijk `5/87` met `6/114`.<br>
				`5/87 approx 0,057` en `6/114 approx 0,053`<br>
				`0,057 &gt; 0,053` dus Jan geeft naar verhouding meer.<br>
				6 is meer dan 5, dus Maja geeft in absolute waarde meer.<br>
				Wat mij betreft is Jan de gulste gever, omdat hij naar verhouding het meeste geeft.
			</p>
		</div>
	</div>
</div>

<!-- theorie C -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie C Per 100</h3>
	<img src="../afbeeldingen/vaar_procenten/bank-building-icon.jpg" alt="bank-building-icon" width="150" class="rightfloat" />
	<p>
		Ineke zegt: 'Ik heb € 1300,- spaargeld. Daarvan krijg ik dit jaar € 65,- rente.'<br>
		 'Oh', zegt Arwin, 'ik heb € 1600,- op de bank. En ik krijg dit jaar € 80,- rente. Ik ben beter af dan jij.'<br>
		Heeft Arwin gelijk?
	</p>
	<p>
		Arwin krijgt weliswaar een groter bedrag aan rente. Maar is hij ook beter af? Om goed te kunnen <b>vergelijken</b> rekenen we uit hoeveel rente ieder krijgt <b>per honderd</b> euro.
	</p>		
</div>

<!-- opdracht 8 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 8 Per honderd</h3>
	<ol>
		<li>
							Neem de verhoudingstabellen over en vul ze verder in.
				<table>
				<tr>
					<td>
						<table class="verhouding">
							<thead>
			<tr>
				<td colspan="7">
					Irene
				</td>
			</tr>
		</thead>
							<tbody>
			<tr class="borderbottom">
				<td><em>rente</em></td>
				<td colspan="2">65</td>
				<td colspan="2"></td>
				<td colspan="2">?</td>	
			</tr>
			<tr>
				<td><em>spaargeld</em></td>
				<td colspan="2">1300</td>
				<td colspan="2"></td>
				<td colspan="2">100</td>
			</tr>			
		</tbody>
							<tfoot>
			<tr>
				<th colspan="2"></th>

				<th colspan="4"><img src="afbeeldingen/vaar_procenten/pijlondernaarrechts.png" height="30px" width="120px" class="centerfloat"><br>÷ ...</th>
				<th></th>
			</tr>
		</tfoot>	
						</table>
					</td>
					<td width="20">
					</td>
					<td>
						<table class="verhouding">
							<thead>
			<tr>
				<td colspan="7">
					Arwin
				</td>
			</tr>
		</thead>
							<tbody>
			<tr class="borderbottom">
				<td><em>rente</em></td>
				<td colspan="2">85</td>
				<td colspan="2"></td>
				<td colspan="2">?</td>	
			</tr>
			<tr>
				<td><em>spaargeld</em></td>
				<td colspan="2">1600</td>
				<td colspan="2"></td>
				<td colspan="2">100</td>
			</tr>			
		</tbody>
							<tfoot>
			<tr>
				<th colspan="2"></th>

				<th colspan="4"><img src="afbeeldingen/vaar_procenten/pijlondernaarrechts.png" height="30px" width="120px" class="centerfloat"><br>÷ ...</th>
				<th></th>
			</tr>
		</tfoot>	
						</table>
					</td>
				</tr>
				</table>
			
		</li>
		<li>
			Hoeveel rente per 100 euro krijgt Irene?
		</li>
		<li>
			Hoeveel rente per 100 euro krijgt Arwin?
		</li>
		<li>
			Wie krijgt dan per 100 euro de meeste rente?
		</li>
	</ol>
	
	<div class="hider">
	<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol>
				<li>
					Neem de verhoudingstabellen over en vul ze verder in.
					<table>
						<tr>
							<td>
								<table class="verhouding">
									<thead>
										<tr>
											<td colspan="7">
												Irene
											</td>
										</tr>
									</thead>
									<tbody>
										<tr class="borderbottom">
											<td><em>rente</em></td>
											<td colspan="2">65</td>
											<td colspan="2"></td>
											<td colspan="2">5</td>	
										</tr>
										<tr>
											<td><em>spaargeld</em></td>
											<td colspan="2">1300</td>
											<td colspan="2"></td>
											<td colspan="2">100</td>
										</tr>			
									</tbody>
									<tfoot>
								<tr>
									<th colspan="2"></th>
									<th colspan="4"><img src="afbeeldingen/vaar_procenten/pijlondernaarrechts.png" height="30px" width="120px" class="centerfloat"><br>÷ 13</th>
								<th></th>
							</tr>
						</tfoot>	
					</table>
							</td>
							<td width="20">
							</td>
							<td>
								<table class="verhouding">
									<thead>
										<tr>
											<td colspan="7">
												Arwin
											</td>
										</tr>
									</thead>
									<tbody>
										<tr class="borderbottom">
											<td><em>rente</em></td>
											<td colspan="2">85</td>
											<td colspan="2"></td>
											<td colspan="2">5,3125</td>	
										</tr>
										<tr>
											<td><em>spaargeld</em></td>
											<td colspan="2">1600</td>
											<td colspan="2"></td>
											<td colspan="2">100</td>
										</tr>			
									</tbody>
									<tfoot>
										<tr>
											<th colspan="2"></th>
											<th colspan="4"><img src="afbeeldingen/vaar_procenten/pijlondernaarrechts.png" height="30px" width="120px" class="centerfloat"><br>÷ 16</th>
											<th></th>
										</tr>
									</tfoot>	
								</table>
							</td>
						</tr>
					</table>
				</li>
				<aside class="tekstballon_theorie">
					Rond geldbedragen af op twee decimalen.
				</aside>
				<li>
					Irene krijgt 5,- euro rente per 100 euro.
				</li>
				<li>
					Arwin krijgt 5,31 euro rente per 100 euro.
				</li>
				<li>
					Arwin krijgt dus de meeste rente.
				</li>
			</ol>		
		</div>
	</div>
</div>

<!-- theorie D -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie D Procent</h3>
	<img src="../afbeeldingen/vaar_procenten/procent-teken.png" alt="procent-teken" width="150" class="rightfloat" />
	<p>
		Als de rente 4 euro per 100 euro is, dan zeggen we: 'de rente is 4 <b>procent</b>,' of 'het rente<b>percentage</b> is 4.'
	</p>
	<p>
		Procent betekent letterlijk <em>per honderd</em>.<br>
		Als notatie gebruiken het %-teken.
	</p>
	<p>
		`30/100` deel betekent 30 op de 100. Dat is 30%.
	</p>
</div>


<!-- opdracht 9 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 9 Welk deel</h3>
	<img src="../afbeeldingen/vaar_procenten/vierkant_procenten.png" alt="vierkant_procenten" width="200" class="rightfloat" />
	<p>
		Het vierkant hiernaast is verdeeld in 100 kleine vierkantjes.
	</p>
	<ol>
		<li>Hoeveel kleine vierkantjes zijn gekleurd?</li>
		<li>Welk deel is dat?</li>
		<li>Welk percentage is dat?</li>
		<li>Welk percentage is niet gekleurd?</li>
	</ol>
	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol>
				<li>25 vierkantjes zijn gekleurd.</li>
				<li>`25/100` deel</li>
				<li>25%</li>
				<li>100% - 25% = 75%<br>
							75% is niet gekleurd.
				</li>
			</ol>
		</div>
	</div>
</div>


<!-- opdracht 10 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 10 Percentages</h3>
	<img src="../afbeeldingen/vaar_procenten/vierkant_percentages.png" alt="vierkant_percentages" width="300" class="rightfloat" />
	<p>
		In het vierkant hiernaast zijn verschillende delen gekleurd. Bepaal per kleur het percentage.
	</p>

	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
				<table>
					<tr>
						<td width="150">
							1% is rood<br>
							2,5% is geel<br>
							5% is paars
						</td>
							<td width="150">
							6% is groen<br>
							10% is bruin<br>
							40% is blauw
						</td>
					</tr>
				</table>
				
		</div>
	</div>	
	
</div>
			
	
<!-- opdracht 11 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 11 Makkelijke percentages</h3>
	<p>Neem over en vul de percentages in. <br>
		Doe het zo:
	</p>
	<p>
			`2/5 = 40/100 = 0,40 = 40%`
	</p>
	
	<table class="breuken">
		<tr>
			<td width="300">
				<ol class="breuken">
					<li>
						`1/2 = .../100 = ... = ...%`
					</li>
					<li>
						`1/5 =  .../100 = ... = ...%`
					</li>
					<li>
						`3/4 = .../100 = ... = ...%`
					</li>
				</ol>
			</td>
			<td width="300">
				<ol start="4" class="breuken">
					<li>
						`1/8 =  (12 1/2)/100 = 0`,`125 = ...%`
					</li>
					<li>
						`5/8 =  .../100 = ... = ...%`
					</li>
					<li>
						`3/5 =  .../100 = ... = ...%`
					</li>
				</ol>
			</td>
		</tr>
	</table>

	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
				<table>
					<tr>
						<td width="200">
							<ol class="breuken">
								<li>
									`1/2 = 50%`
								</li>
								<li>
									`1/5 = 20%`
								</li>
								<li>
									`3/4 = 75%`
								</li>
							</ol>
						</td>
						<td width="200">
							<ol start="4" class="breuken">
								<li>
									`1/8 = 12`,`5%`
								</li>
								<li>
									`5/8 = 62`,`5%`
								</li>
								<li>
									`3/5 = 60%`
								</li>
							</ol>
						</td>
					</tr>
				</table>
		</div>
	</div>	
	
</div>	
			

<!-- theorie E -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie E Percentage berekenen</h3>
	<p>
		9% betekent 9 op de 100. In een klas met 100 leerlingen vormen 9 jongens ook precies 9% van de klas. Maar wat nu als je een klas hebt van 38 leerlingen?
	</p>
	<p>
		Hoe bereken je het percentage jongens als er 9 jongens is een klas van 38 leerlingen zitten? 
	</p>
	<p>
		Bereken hoeveel procent het <b>deel</b> van het <b>geheel</b> is. In dit voorbeeld zijn de jongens het deel en is de klas het geheel.
	</p>
		<table class="stappenplan">
			<tr>
				<td>
					Stap 1
				</td>
				<td width="180">
					Maak de verhoudingstabel,<br>
					zet boven het deel en onder het geheel.
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel: jongens</em></td>
								<td colspan="2">...</td>
								<td colspan="2">...</td>
								<td colspan="2">...</td>
							</tr>
							<tr>
								<td><em>geheel: klas</em></td>
								<td colspan="2">...</td>
								<td colspan="2">...</td>
								<td colspan="2">...</td>
							</tr>
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>÷ ...</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>× ...</th>
								<th></th>
							</tr>
						</tfoot>
					</table>
				</td>
			</tr>
			<tr>
				<td>
					Stap 2
				</td>
				<td width="180">
					Zet de getallen in de tabel.
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel: jongens</em></td>
								<td colspan="2">9</td>
								<td colspan="2">...</td>
								<td colspan="2">...</td>
							</tr>
							<tr>
								<td><em>geheel: klas</em></td>
								<td colspan="2">38</td>
								<td colspan="2">1</td>
								<td colspan="2">...</td>
							</tr>
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>÷ ...</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>× ...</th>
								<th></th>
							</tr>
						</tfoot>
					</table>
				</td>
			</tr>
			<tr>
				<td>
					Stap 3
				</td>
				<td width="180">
					Zet onder de 1 en 100%. <br> Zet rechtsboven ?%.
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel: jongens</em></td>
								<td colspan="2">9</td>
								<td colspan="2">...</td>
								<td colspan="2">...%</td>
							</tr>
							<tr>
								<td><em>geheel: klas</em></td>
								<td colspan="2">38</td>
								<td colspan="2">1</td>
								<td colspan="2">100%</td>
							</tr>
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>÷ ...</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>× ...</th>
								<th></th>
							</tr>
						</tfoot>
					</table>
				</td>
			</tr>
			<tr>
				<td>
					Stap 4
				</td>
				<td width="180">
					Bepaal de route van 38 naar 100 via 1 en bereken ?
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ 38<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel: jongens</em></td>
								<td colspan="2">9</td>
								<td colspan="2">...</td>
								<td colspan="2">24%</td>
							</tr>
							<tr>
								<td><em>geheel: klas</em></td>
								<td colspan="2">38</td>
								<td colspan="2">1</td>
								<td colspan="2">100%</td>
							</tr>
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>÷ 38</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px"><br>× 100</th>
								<th></th>
							</tr>
						</tfoot>
					</table>
				</td>
			</tr>
		</table>
		<p>
			Het percentage jongens is 24%.<br>
			In feite heb je die 24% als volgt uitgerekend:
			`9/38 &times; 100 approx 24%`
		</p>		
</div>


<!-- opdracht 12 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 12 Percentage</h3>
	<p>Klas 3.4 van het Novum College bestaat uit 24 leerlingen. Er zitten 17 meisjes in de klas.<br>
		Bereken het percentage meisjes in klas 3.4 met behulp van de gegeven verhoudingstabel. Rond af op één decimaal.
	</p>
	<table class="verhouding" style="background-color:white">
		<thead>
			<tr>
				<th colspan="2"></th>
				<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><em>deel: meisjes</em></td>
				<td colspan="2">...</td>
				<td colspan="2">...</td>
				<td colspan="2">...%</td>
			</tr>	
			<tr class="borderbottom">
				<td><em>geheel: klas</em></td>
				<td colspan="2">...</td>
				<td colspan="2">1</td>
				<td colspan="2">100</td>	
			</tr>
					
		</tbody>
		<tfoot>
			<tr>
				<th colspan="2"></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
	
		<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<table class="verhouding" style="background-color:white">
				<thead>
					<tr>
						<th colspan="2"></th>
						<th colspan="2">÷ 24<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
						<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td><em>deel: meisjes</em></td>
						<td colspan="2">17</td>
						<td colspan="2">...</td>
						<td colspan="2">70,8%</td>
					</tr>	
					<tr class="borderbottom">
						<td><em>geheel: klas</em></td>
						<td colspan="2">24</td>
						<td colspan="2">1</td>
						<td colspan="2">100</td>	
					</tr>
							
				</tbody>
				<tfoot>
					<tr>
						<th colspan="2"></th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 24</th>
						<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
						<th></th>
					</tr>
				</tfoot>	
			</table>
			<p>
				De meisjes vormen 70,8% van de klas.
			</p>		
		</div>
		</div>
	
</div>


<!-- opdracht 13 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 13 Percentages berekenen</h3>
	<p>
		Maak de volgende opgaven met behulp van een verhoudingstabel.
	</p>
	<ol>
		<li>
			In een mand met 16 appels zijn er 5 rot. Hoe groot is het percentage rotte appels?
		</li>
		<li>
			Aan een kapstok hangen 3 rode, 2 witte en 4 zwarte jassen. Hoe groot is het percentage witte jassen?
		</li>
		<li>
			117 leerlingen worden in het donker gedropt. 42 leerlingen zijn op tijd terug. Welk percentage leerlingen is te laat?
		</li>
	</ol>

	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol>
				<li>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ 16<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel</em></td>
								<td colspan="2">5</td>
								<td colspan="2">...</td>
								<td colspan="2">31,3%%</td>
							</tr>	
							<tr class="borderbottom">
								<td><em>geheel</em></td>
								<td colspan="2">16</td>
								<td colspan="2">1</td>
								<td colspan="2">100</td>	
							</tr>
				
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 16</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
								<th></th>
							</tr>
						</tfoot>	
					</table>
					<p>
						Het percentage rotte appels is 31,3% 
					</p>
				</li>
				<li>
					<p>
						Het geheel is het totaal aantal jassen.<br>
						`3 + 2 + 4 = 9`
					</p>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ 9<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel</em></td>
								<td colspan="2">2</td>
								<td colspan="2">...</td>
								<td colspan="2">22,2%</td>
							</tr>	
							<tr class="borderbottom">
								<td><em>geheel</em></td>
								<td colspan="2">9</td>
								<td colspan="2">1</td>
								<td colspan="2">100</td>	
							</tr>
									
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 9</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
								<th></th>
							</tr>
						</tfoot>	
					</table>
					<p>
						Het percentage witte jassen is 22,2%
					</p>
				</li>
				<li>
					<p>
						Als van de 117 er 42 op tijd zijn, dan zijn (117 - 42 =) 75 leerlingen te laat.
					</p>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ 117<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel</em></td>
								<td colspan="2">75</td>
								<td colspan="2">...</td>
								<td colspan="2">64,1%</td>
							</tr>	
							<tr class="borderbottom">
								<td><em>geheel</em></td>
								<td colspan="2">117</td>
								<td colspan="2">1</td>
								<td colspan="2">100</td>	
							</tr>
									
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 117</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
								<th></th>
							</tr>
						</tfoot>	
					</table>
					<p>
						64,1% van de leerlingen is te laat.
					</p>
				</li>
			</ol>
		</div>
	</div>
</div>


<!-- theorie F Vergelijken met behulp van percentages -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie F Van breuk naar percentage</h3>
	<p>
		Met een verhoudingstabel kun je van een breuk een percentage maken.
	</p>
		<aside class="tekstballon_theorie">
		Een verhouding is een breuk, want<br>
		`3 : 4 = 3/4`
	</aside>
	<p>
		Als voorbeeld berekenen we hoeveel procent 9 jongens in een klas van 38 zijn.<br>
		Reken de breuk `9/38` om naar procenten, zie de verhoudingstabel hieronder.
	</p>	
	<table class="verhouding" style="background-color:white">
		<thead>
			<tr>
				<th colspan="2"></th>
				<th colspan="2">÷ 38<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><em>deel: jongens</em></td>
				<td colspan="2">9</td>
				<td colspan="2">...</td>
				<td colspan="2">24%</td>
			</tr>	
			<tr class="borderbottom">
				<td><em>geheel: klas</em></td>
				<td colspan="2">38</td>
				<td colspan="2">1</td>
				<td colspan="2">100</td>	
			</tr>
					
		</tbody>
		<tfoot>
			<tr>
				<th colspan="2"></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 38</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>

	<p>
		 De rekenstappen achter elkaar, geven de volgende berekening:
	</p>
	<p>
		`9/38 &times; 100 approx 24%`<br>
	</p>
	<p>
		Een breuk met `100` vermenigvuldigen, geeft in één keer het percentage.
	</p> 


		<aside class="tekstballon_theorie">
		Merk op: tenzij anders gevraagd, rond je percentages standaard af op één decimaal achter de komma.
	</aside>
	
<p>
	Voorbeeld:<br>
	In een mand met 24 appels, zijn er 7 rot. Bereken het percentage rotte appels.
	</p>
	<p>
	Uitwerking:<br>
		`7/24 &times; 100 approx 29,2%`
	</p>
	
</div>


<!-- opdracht 14 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 14 Van breuk naar percentage</h3>
	<p>
		Druk de volgende breuken uit als percentage. Neem de breuken steeds eerst over.
	</p>
	<table>
		<tr>
			<td width="300">
				<ol class="breuken">
					<li>
						`5/40`
					</li>
					<li>
						`8/32`
					</li>
					<li>
						`9/100`
					</li>
				</ol>
			</td>
			<td width="300">
			<ol start="4" class="breuken">
				<li>
					`23 /120`
				</li>
				<li>
					`45/329`
				</li>
				<li>
					`90/155`
				</li>
			</ol>
			</td>
		</tr>
	</table>
				
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<table>
				<tr>
					<td width="300">
						<ol class="breuken">
							<li>
								`5/40 = 12,5%`
							</li>
							<li>
								`8/32 = 25%`
							</li>
							<li>
								`9/100 = 9%`
							</li>
						</ol>
					</td>
					<td width="300">
					<ol start="4" class="breuken">
						<li>
							`23 /120 approx 19,2% `
						</li>
						<li>
							`45/329 approx 13,7%`
						</li>
						<li>
							`90/155 approx 58,1%`
						</li>
					</ol>
					</td>
				</tr>
			</table>
		</div>
	</div>
</div>


<!-- opdracht 15 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 15 Percentages</h3>
			<aside class="tekstballon_theorie">
		Merk op: tenzij anders gevraagd, rond je percentages standaard af op één decimaal achter de komma.
	</aside>
	<p>
		Maak in de volgende opgaven eerst een breuk van de verhouding en geef dan het bijbehorende percentage. Schrijf je berekeningen op.
	</p>
	<ol>
		<li>
			Van 18 leerlingen zijn er 5 te laat. Welk percentage is te laat?
		</li>
		<li>
			In de wei lopen 132 merries en 5 hengsten. Hoeveel procent van de dieren is vrouwelijk?
		</li>
		<li>
			Op mijn verjaardag kreeg van 11 gasten een cadeau, de anderen 3 gaven mij niets. Hoeveel procent gaf mij geen cadeau?
		</li>
	</ol>
				
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol class="breuken">
				<li>
					`5/18` wordt `5/18 * 100% approx 27,8%` 
				</li>
				<li>
					`132/137` wordt `132/137 * 100% approx 96,4%` 
				</li>
				<li>
					`3/14` wordt `3/14 * 100% approx 21,4%` 
				</li>
			</ol>

		</div>
	</div>
</div>


<!-- opdracht 16 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 16 Verhoudingen, breuken en percentages</h3>
		<p>
		Bekijk de uitleg van Roeland Hiele op www.wiskunde-academie.nl over procenten en percentages:
		<iframe width="560" height="315" src="https://www.youtube.com/embed/xcezUzUjS5M" frameborder="0" allowfullscreen></iframe>
		<br>
		Druk de volgende verhoudingen uit als breuk, decimaal getal en percentage:
	</p>
	<ol>
		<li>
			25 : 210
		</li>
		<li>
			120 : 32
		</li>
		<li>
			90 : 179
		</li>
	</ol>
		
		
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol class="breuken">
				<li>
					`25 : 210 = 25/210 approx 0,119048 approx 11,9%`
				</li>
				<li>
					`120 : 32 = 120/32 = 3,75 = 375%`
				</li>
				<li>
					`90 : 179 = 90/179 approx 0,502793 approx 50,3%` 
				</li>
			</ol>
		</div>
	</div>
</div>
	

<!-- opdracht 17 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 17 Hoeveel procent rente</h3>
	<p>
		Vorig jaar had Gjalt 900 euro. Hij kreeg 18 euro rente.
<br>
		Bereken hoeveel procent de rente was, zonder verhoudingstabel. Schrijf je berekening op.
	</p>
	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<p>
				`18/900 * 100% = 2%`<br>
			</p>
		</div>
	</div>
</div>


<div class="tekstblok opdracht">
	<h3>Opdracht 18 Doelsaldo</h3>
	<p>
		Paul speelt hockey. Afgelopen wedstrijd maakte hij 3 van de 11 doelpunten. 
		<br>
		Hoeveel procent van de doelpunten heeft hij gemaakt? Schrijf je berekening op.
	</p>
	
		<div class="hider">
			<div class="showhide">&rarr;</div>
			<div class="antwoord">
				<p>
					`3/11 * 100% approx 27,3%`
				</p>
			</div>
		</div>
		
			
</div>


<!-- theorie G Vergelijken met behulp van percentages -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie G Vergelijken met behulp van percentages</h3>
	
		<aside class="tekstballon_theorie">
		Jan geeft 5 van 87.<br>
		`5/87 &times; 100% approx 5`,`7%`<br>
		Maja geeft 6 van 114.<br>
		`6/114 &times; 100% approx 5`,`3%`
	</aside>
	
	<p>
		Bij opdracht 7 vergeleken we de giften van Jan en Maja. Uitgedrukt in percentages wordt meteen duidelijk wie naar verhouding het meeste geeft:
	</p>
	<p>
		Jan geeft 5,7% en Maja geeft 5,3%. Dus Jan geeft naar verhouding het meest.
	</p>
	
</div>



<!-- opdracht 19 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 19 Slagingspercentage</h3>
	<p>
		Op het Kromme Waal College slagen 11 van de 90 leerlingen voor het examen. Van de 83 kandidaten op het Marnix Lyceum slagen er 10.  
	</p>
	<p>
		Op welke school is het slagingspercentage het hoogst? Schrijf je berekening op.
	</p>
	
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<p class="breuken">
				`11/90 * 100 approx 12,2%`<br>
				`10/83 * 100 approx 12,0%`<br>
				Dus op het Kromme Waal College is het slagingspercentage het hoogst. 
				
			</div>
	</div>
</div>


<!-- opdracht 20 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 20 Eiwitten</h3>
	<p>
		Op de verpakking van een pak hagelslag staat dat je ongeveer 20 gram hagelslag op één boterham doet. Verder kun je er lezen dat in 100 gram hagelslag 4,9 gram eiwit zit.<br>
Volgens de verpakking van chocopasta smeer je 15 gram op één boterham. In 100 gram chocopasta zit 6,5 gram eiwit.
	</p>
	<p>
		Bereken het verschil in eiwitgewicht tussen een boterham met hagelslag en een boterham met chocopasta. 
	</p>
	
	<div class="hider">
			<div class="showhide">&rarr;</div>
			<div class="antwoord">
				<p class="breuken">
					Boterham met hagelslag:<br>
					`20/100 * 4,9 = 0,98` gram eiwit<br>
					Boterham met chocopasta:<br>
					`15/100 * 6,5 = 0,975` gram eiwit<br>
					Het verschil is `0,98 - 0975 = 0,005` gram eiwit.
		</div>
	</div>
	
</div>


<!-- opdracht 21 -->
<div class="tekstblok opdracht">
	<h3>Opdracht 21 Kansen (havo/vwo)</h3>
	<p>
		Kansen worden in het dagelijks taalgebruik helaas vaak uitgedrukt in procenten.<br>Bijvoorbeeld de kans dat 2 op de tien slagen, is `2/10 * 100 = 20%`.
	</p>
	<ol>
		<li>
			Hoe groot is de kans dat je een even aantal ogen gooit met één dobbelsteen?
		</li>
		<li>
			Hoe groot is de kans dat je een vijf gooit met één dobbelsteen?
		</li>
		<li>
			Hoe groot is de kans dat je hoger dan een twee gooit met één dobbelsteen?
		</li>
		<li>
			De kans dat je de hoofdprijs van de Staatloterij wint is maximaal 1 op 5 miljoen. Druk die kans uit in een percentage.
		</li> 
	</ol> 
		
	<div class="hider">
			<div class="showhide">&rarr;</div>
			<div class="antwoord">
				<ol class="breuken">
					<li>
						`3/6 * 100% = 50%`, want `3` van de `6` mogelijkheden zijn even (`2`, `4` en `6`).
					</li>
					<li>
						`1/6 * 100% approx 16,7%`, want `1` van `6` mogelijkheden is `5`.
					</li>
					<li>
						`4/6 * 100% approx 66,7%`, want `4` van de `6` mogelijkheden zijn groter dan `2` (`3`, `4`, `5` en `6`).
					</li>
					<li>
						`1/5000000 * 100% = 0,00002%`
					</li> 
				</ol> 
		</div>
	</div>
	
</div>


<!-- theorie H -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie H Van percentage naar getal</h3>

	<p>
		Hoe bereken je het aantal jongens in een klas als je weet dat er ongeveer 30% jongens zitten in een klas van 26. 
	</p>
	<p>
		In dit voorbeeld zijn de jongens het deel en is de klas het geheel. Te berekenen is het aantal jongens. Maak gebruik van een verhoudingstabel.<br>
		De jongens vormen ongeveer 30% van het geheel. De hele klas is 100%.
	</p>
		<table class="stappenplan">
			<tr>
				<td>
					Stap 1
				</td>
				<td width="180">
					Zet de jongens als deel boven de klas als geheel in de tabel. We zoeken de waarde van "?".
				</td>
				<td>
					
		<table class="verhouding" style="background-color:white">
		<thead>
			<tr>
				<th colspan="2"></th>
				<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><em>deel: jongens</em></td>
				<td colspan="2">30</td>
				<td colspan="2">...</td>
				<td colspan="2">?</td>
			</tr>	
			<tr class="borderbottom">
				<td><em>geheel: klas</em></td>
				<td colspan="2">100</td>
				<td colspan="2">...</td>
				<td colspan="2">...</td>	
			</tr>
					
		</tbody>
		<tfoot>
			<tr>
				<th colspan="2"></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× ...</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
				</td>
			</tr>
			<tr>
				<td>
					Stap 2
				</td>
				<td width="180">
					Zet het aantal van de klas in de tabel en de `1` er tussen in de onderste rij.
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
		<thead>
			<tr>
				<th colspan="2"></th>
				<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><em>deel: jongens</em></td>
				<td colspan="2">30</td>
				<td colspan="2">...</td>
				<td colspan="2">?</td>
			</tr>	
			<tr class="borderbottom">
				<td><em>geheel: klas</em></td>
				<td colspan="2">100</td>
				<td colspan="2">1</td>
				<td colspan="2">26</td>	
			</tr>
					
		</tbody>
		<tfoot>
			<tr>
				<th colspan="2"></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ ...</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× ...</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
				</td>
			</tr>
			<tr>
				<td>
					Stap 3
				</td>
				<td width="180">
					Bepaal de route van `100` via `1` naar `26`.
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
		<thead>
			<tr>
				<th colspan="2"></th>
				<th colspan="2">÷ ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× ...<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><em>deel: jongens</em></td>
				<td colspan="2">30</td>
				<td colspan="2">...</td>
				<td colspan="2">?</td>
			</tr>	
			<tr class="borderbottom">
				<td><em>geheel: klas</em></td>
				<td colspan="2">100</td>
				<td colspan="2">1</td>
				<td colspan="2">26</td>	
			</tr>
					
		</tbody>
		<tfoot>
			<tr>
				<th colspan="2"></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 100</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 26</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
				</td>
			</tr>
				<tr>
				<td>
					Stap 4
				</td>
				<td width="180">
					Bereken het aantal jongens, gebruik zonodig je rekenmachine.
				</td>
				<td>
					<table class="verhouding" style="background-color:white">
						<thead>
							<tr>
								<th colspan="2"></th>
								<th colspan="2">÷ 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th colspan="2">× 26<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td><em>deel: jongens</em></td>
								<td colspan="2">30</td>
								<td colspan="2">...</td>
								<td colspan="2">7,8</td>
							</tr>	
							<tr class="borderbottom">
								<td><em>geheel: klas</em></td>
								<td colspan="2">100</td>
								<td colspan="2">1</td>
								<td colspan="2">26</td>	
							</tr>
									
						</tbody>
						<tfoot>
							<tr>
								<th colspan="2"></th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 100</th>
								<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 26</th>
								<th></th>
							</tr>
						</tfoot>	
					</table>
				</td>
			</tr>
		</table>
		<p>
			Het aantal jongens is afgerond 8.<br>
			In feite heb je die 8 als volgt uitgerekend:
			`30/100 &times; 26 approx 8`
		</p>		
</div>


<!-- opdracht 22-->
<div class="tekstblok opdracht">
	<h3>Opdracht 22</h3>
	<p>
		In een klas van 38 leerlingen is ongeveer 40% een meisje.
	</p>
	<ol>
		<li>
			Wat is het percentage jongens?
		</li>
		<li>
			Bereken het aantal meisjes en het aantal jongens.
		</li>
	</ol>
		
		<div class="hider">
			<div class="showhide">&rarr;</div>
			<div class="antwoord">
				<ol>
					<li>
						`100% - 40% = 60%`<br>
						De jongens vormen ongeveer 60% van deze klas.
					</li>
					<li>
						<table class="verhouding" style="background-color:white">
							<thead>
								<tr>
									<th colspan="2"></th>
									<th colspan="2">÷ 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th colspan="2">× 38<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
									<th></th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td><em>deel: meisjes</em></td>
									<td colspan="2">40</td>
									<td colspan="2">...</td>
									<td colspan="2">15,2</td>
								</tr>	
								<tr class="borderbottom">
									<td><em>geheel: klas</em></td>
									<td colspan="2">100</td>
									<td colspan="2">1</td>
									<td colspan="2">38</td>	
								</tr>
										
							</tbody>
							<tfoot>
								<tr>
									<th colspan="2"></th>
									<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 100</th>
									<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 38</th>
									<th></th>
								</tr>
							</tfoot>	
						</table>
						<p>
							`38-15=23`<br>
							Er zijn 15 meisjes in de klas en 23 jongens. 
						</p>
					</li>
		</div>
	</div>
	
</div>


<!-- opdracht 23-->
<div class="tekstblok opdracht">
	<h3>Opdracht 23</h3>
	<p>
		Uit een onderzoek naar het woon-werk verkeer onder de medewerkers van de V&D komt naar voren dat 8% lopend naar het werk komt, 62% met de fiets en de rest met het openbaar vervoer.
	</p>
	<ol>
		<li>
			Bereken het aantal medewerkers dat lopend naar het werk gaat in een filiaal waar 125 medewerkers werken. Schrijf je berekening op.
		</li>
		<li>
			Hoeveel medewerkers fietsen naar hun werk gaan, als een filiaal 50 medewerkers heeft. Schrijf je berekening op.
		</li>
		<li>
			Het grootste filiaal telt 190 personeelsleden. Hoeveel van hen komt met het openbaar vervoer naar het werk? Schrijf je berekening op.
		</li>
	</ol>
		 
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol class="breuken">
				<li>
					`8/100 * 125 = 10`<br>
					Van 125 medewerkers gaan er tien lopend naar het werk. 
				</li>
				<li>
					`62/100*50=31`
					Van 50 medewerkers gaan er 31 op de fiets.
				</li>
				<li>
					`100% - 8% - 62% = 30%`<br>
					`30/100 * 190 = 57`<br>
					30% van de 190 medewerkers komt overeen met 57 personen.<br>
				</li>
			</ol>
		</div>
	</div>
	
</div>



<!-- opdracht -->
<div class="tekstblok opdracht">
	<h3>Opdracht 24 Inval politie (havo/vwo)</h3>
	<p>
		<img src="../afbeeldingen/vaar_procenten/inval_afbeelding.jpeg" alt="inval_afbeelding" width="250" class="rightfloat" />
		Bij een inval van de politie in een hennepkwekerij worden 4 personen aangehouden. Naar schatting vormen zij slechts 16% van de organisatie rond de kwekerij.
	</p>
	<p>
		Bereken uit hoeveel personen de organisatie bestaat, als de schatting zou kloppen. Schrijf je berekening op.
	</p>
		
		<div class="hider">
			<div class="showhide">&rarr;</div>
			<div class="antwoord">
				<p class="breuken">
					Stel het aantal personen van de organisatie op `x`, dan geldt dat:<br>
					`16%` van `x` gelijk is aan 4.<br>
					Dat leidt tot de volgende vergelijking:	`16/100 * x = 4` <br>
					`16x = 400` (beide kanten met 100 vermenigvuldigen)<br>
					`x = 400/16 = 25` (beide kanten door 16 delen)<br>
					Als de schatting zou kloppen, bestaat de organisatie uit 25 personen.
				</p>
		</div>
	</div>
	
</div>



<!-- theorie C -->
<div class="tekstblok theorie vaardigheden">
	<h3>Theorie I Van deel naar geheel</h3>
	<p>
		Als je een percentage weet, kun je een geheel berekenen, zie onderstaand voorbeeld.
	</p>
	<p><img src="../afbeeldingen/vaar_procenten/garage_afbeelding.jpg" alt="garage_afbeelding" class="rightfloat" width="150" height="" />
		In parkeergarage <em>De Vest</em> staan 18 auto's al langer dan een jaar geparkeerd. Deze zogenoemde 'vergeten' auto's nemen 3% van de beschikbare parkeergelegenheid in. <br>
		Bereken het totale aantal parkeerplaatsen in <em>De Vest</em>.
	</p>
	<p>
		In een verhoudingstabel worden de gegeven aantallen en percentages gezet. 18 parkeerplaatsen komen overeen met 3%. Berekend moet worden welk aantal bij alle auto's hoort: het vraagteken in de tabel.
	</p>
	<aside class="tekstballon_theorie" style="width: 250px !important">
		Het getal boven de `1` schrijf je niet op. <br>Gebruik zo nodig een rekenmachine.
	</aside>
		
	<table class="verhouding" style="background-color:white">
		<thead>
			<tr>
				<th colspan="2"></th>
				<th colspan="2">÷ 3<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th colspan="2">× 100<br><img src="afbeeldingen/lrpijlboven.png" height="10px" class="centerfloat"></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			<tr class="borderbottom">
				<td><em>deel plaatsen</em></td>
				<td colspan="2">18</td>
				<td colspan="2">...</td>
				<td colspan="2">?</td>	
			</tr>
			<tr>
				<td><em>geheel</em></td>
				<td colspan="2">3%</td>
				<td colspan="2">1%</td>
				<td colspan="2">100%</td>
			</tr>			
		</tbody>
		<tfoot>
			<tr>
				<th colspan="2"></th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>÷ 3</th>
				<th colspan="2"><img src="afbeeldingen/lrpijlonder.png" height="10px" class="centerfloat"><br>× 100</th>
				<th></th>
			</tr>
		</tfoot>	
	</table>
	
	<p>
		De route van 3% naar 100% is makkelijk: eerst delen door 3 en dan vermenigvuldigen met 100. Dezelfde route levert het gevraagde totale aantal parkeerplaatsen op:
	</p>
	<p>
	`18/3 &times; 100 = 600`<br>
	Er zijn 600 parkeerplaatsen in <em>De Vest</em>.
	</p>
</div>


<!-- opdracht -->
<div class="tekstblok opdracht">
	<h3>Opdracht 25 Van deel naar totaal</h3>
	<ol>
		<li>
			7 appels vormen ongeveer 60% van het aantal stukken fruit in een fruitschaal. Hoeveel stukken 
		fruit liggen er in de schaal? Rond af op hele stukken fruit en schrijf je berekening op.
		</li>
		<li>
			56 groene fietsen vormen ongeveer 32% van het totaal aantal fietsen in de stalling. Hoeveel fietsen staan er in totaal? Rond af op hele fietsen en schrijf je berekening op.
		</li>
		<li>
			9 jongens kopen ieder 3 ijsjes. Samen hebben zij ongeveer 73% van de ijsjes op gekocht. Hoeveel ijsjes waren er ongeveer op school te koop? Rond af op hele ijsjes en schrijf je berekening op.
		</li> 
	</ol>
		
		<div class="hider">
			<div class="showhide">&rarr;</div>
			<div class="antwoord">
				<ol class="breuken">
					<li>
						`7 : 60 * 100 = 11 2/3`<br>
						Afgerond op hele stukken fruit liggen er 12 stukken fruit in de schaal.
					</li>
					<li>
						`56/32 * 100 = 175`<br>
						Er staan 175 fietsen in totaal.
					</li>
					<li>
						`9 * 3 = 27`. Dus de jongens kopen samen 27 ijsjes.<br>
						`27/73 * 100 = 36 72/73`<br>
						Er waren ongeveer 37 ijsjes te koop.
					</li> 
				</ol>
		</div>
	</div>
	
</div>


<!-- opdracht -->
<div class="tekstblok opdracht">
	<h3>Opdracht 26 Totalen (havo/vwo)</h3>

	<p>
		Geef steeds een berekening bij het antwoord. Bedenk naar aanleiding van de vraag, hoe je moet afronden.
	</p>
		
	<ol>
		<li>
			Van een groep mensen zijn er 7 linkshandig. Zij vormen bijna 3% van de groep.
			Bereken de totale groepsgrootte.
		</li> 
		<li>
			Bij ijssalon <em>Het bevroren koekje</em> is ongeveer 5% van de smaken uitverkocht. Het gaat om de smaken <em>appel</em>, <em>rum-rozijnen</em> en <em>kiwi</em>. Hoeveel smaken heeft <em>Het bevroren koekje</em> normaal in de verkoop?
		</li>
		<li>
			Een voetbalwedstrijd duurt standaard 90 minuten. Een wedstrijd kan langer duren, bijvoorbeeld door verlenging van 30 minuten en blessuretijd.<br>
			Bij een voetbalwedstrijd met verlenging zat Gjalt 4,72% van de wedstrijd aan de kant. Dat waren in totaal 6 minuten. Hoeveel blessuretijd werd er gegeven?
		</li>
	</ol>
			
	<div class="hider">
		<div class="showhide">&rarr;</div>
		<div class="antwoord">
			<ol>
				<li>
					`7/3 * 100 = 233 1/3`<br>
					De groep bestaat uit 233 personen.
				</li> 
				<li>
					3 smaken vormen 5% van het geheel.<br>
					`3/5 * 100 = 60`<br>		
					Normaal heeft de ijssalon 60 smaken in de verkoop.
				</li>
				<li>
					Als 4,72% overeenkomt met 6 minuten, dan duurde de totale wedstrijd<br>
					`6/(4,72) * 100 approx 127 ` minuten.<br>
					Een standaard wedstrijd met verlening duurt 120 minuten (90 + 30).<br>
					Er werd dus ongeveer 7 minuten blessuretijd gegeven (127 - 120).
				</li>
			</ol>
	</div>
	</div>
	
</div>

<div class="tekstblok theorie vaardigheden">
	<p>
		De volgende vaardigheden taak is: <a href="http://wiskunde.vathorstcollege.nl/index2.php?p=vaar_procentuele_groei">Procentuele groei</a>.
	</p>
</div>


</div>
</div>